...
for run = 1:20
 ...
 %% testing
    ...
    position = find(mistakeTestMatrix);
    for i = 1:size(position,1)
        Confusion{run}(Yvalue_test(position(i)),Ytest(position(i))+1) = Confusion{run}(Yvalue_test(position(i)),Ytest(position(i))+1)+1;
    end
end

ConfusionOutput = cell(10);
tem = zeros(20,1);
for i = 1:10
    for j = 1:10
        for k = 1:20
            tem(k) = Confusion{k}(i,j);
        end
        ConfusionOutput{i,j} = [mean(tem),std(tem)]; 
    end
end
